...
for qualitative:
    3D-CODED : 3D Correspondences by Deep Deformation
        https://openaccess.thecvf.com/content_ECCV_2018/papers/Thibault_Groueix_Shape_correspondences_from_ECCV_2018_paper.pdf
    
    ZoomOut: Spectral Upsampling for Efficient Shape Correspondence
        https://arxiv.org/pdf/1904.07865.pdf
for both:
    Unsupervised Learning of Dense Shape Correspondence:
        Trained on the Faust synthetic or scanned data, test on Tosca
        https://openaccess.thecvf.com/content_CVPR_2019/papers/Halimi_Unsupervised_Learning_of_Dense_Shape_Correspondence_CVPR_2019_paper.pdf
        They do not provide numbers
    Neural Subdivision
        train on Centaur, test on others
        train on Centaur, David, Horse, test on others
        https://arxiv.org/pdf/2005.01819.pdf
"""
import torch, glob
...
class ToscaDataset(Matching_Dataset):
    ...
    def processed_file_name(self):
        return self.processed_folder_name() + "TOSCA_p{}_r{}_{}".format(
    ...
    def get_process_scan_list(self):
        ...
        # get all classes
        files = glob.glob(os.path.join(self.root, "*.mat"))
    ...
    def process_impl(self, key):
        ...
        def get_model(idx):
            mesh = load_mat2mesh(os.path.join(self.root, idx + ".mat"))
            return mesh

        mesh_a = get_model(self.seqmap[cls_a][idx_a])
        mesh_p = get_model(self.seqmap[cls_p][idx_p])

        # o3d.io.write_triangle_mesh("tosca_hourse_a.ply",mesh_a)
        # o3d.io.write_triangle_mesh("tosca_hourse_p.ply",mesh_p)

        if self.target_points > 0:
            mesh_a, mesh_p = simplify_mesh([mesh_a, mesh_p], self.target_points, False)

        # o3d.io.write_triangle_mesh("tosca_hourse_a_f.ply",mesh_a)
        # o3d.io.write_triangle_mesh("tosca_hourse_p_f.ply",mesh_p)
...
if __name__ == "__main__":
    from configs import training_configs, TOSCA_DIR

    settings = training_configs()
    # dataset = ToscaDataset(TOSCA_DIR,
    #                        train=True,
    #                        mode =settings.data_mode,
    #                        target_shape='random')
    dataset = ToscaDataset(
        TOSCA_DIR, train=False, mode=settings.data_mode, target_shape="cat"
    )
    # dataset = ToscaDataset(TOSCA_DIR,
    #                        train=False,
    #                        mode =settings.data_mode,
