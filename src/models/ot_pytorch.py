...
def sink(M, reg, numItermax=1000, stopThr=1e-9, cuda=True):
    ...
    if cuda:
        a = (torch.ones((M.size()[0],)) / M.size()[0]).cuda()
        b = (torch.ones((M.size()[1],)) / M.size()[1]).cuda()
    else:
    ...
    return torch.sum(u.view((-1, 1)) * K * v.view((1, -1)) * M), transp
...
def sink_stabilized(
    ...
    if cuda:
        u, v = (
            Variable(torch.ones(na) / na).cuda(),
            Variable(torch.ones(nb) / nb).cuda(),
        )
    else:
    ...
    while loop:
        ...
        if (
            torch.max(torch.abs(u)).data[0] > tau
            or torch.max(torch.abs(v)).data[0] > tau
        ):
            alpha, beta = alpha + reg * torch.log(u), beta + reg * torch.log(v)

            if cuda:
                u, v = (torch.ones(na) / na).cuda(), (torch.ones(nb) / nb).cuda()
            else:
